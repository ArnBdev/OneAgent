...
export class MultiAgentMCPServer {  private communicationProtocol: AgentCommunicationProtocol;
  ...
  constructor(
    ...
    console.log('✅ OURA v3.0: Clean agent registry architecture - no legacy discovery spam!');
  }/**
  ...
  private setupAutomatedDiscovery(): void {
    ...
    });    // Start periodic discovery broadcasts - OURA v3.0: Configurable via .env
    // Legacy broadcast discovery can be disabled via OURA_V3_BROADCAST_DISCOVERY_DISABLED=true
    if (this.autoDiscoveryEnabled) {
      this.startPeriodicDiscovery();
      console.log('� Legacy broadcast discovery ENABLED - set OURA_V3_BROADCAST_DISCOVERY_DISABLED=true to disable');
    } else {
      console.log('✅ OURA v3.0: Broadcast spam eliminated - UnifiedAgentRegistry is single source of truth');
    }    if (process.env.OURA_V3_BROADCAST_DISCOVERY_DISABLED === 'true') {
      console.log('✅ OURA v3.0 Agent Registry Protocol ready!');
      console.log('📋 UnifiedAgentRegistry is the single source of truth for agent discovery');
      console.log('🔇 Legacy "Who\'s awake?" broadcasts disabled - no more spam!');
    } else {
      console.log('✅ Legacy discovery protocol ready!');
