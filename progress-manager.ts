...
interface PhaseProgress {
  ...
  endDate?: string;
  tasks: TaskStatus[];
  kpis: { [key: string]: { target: number | string; current: number | string } };
}
...
class OneAgentProgressManager {
  private projectFile = path.join(__dirname, 'project-status.json');
  private trackerFile = path.join(__dirname, 'ONEAGENT_IMPLEMENTATION_PROGRESS_TRACKER.md');
  ...
  private initializeProject(): void {
    if (!fs.existsSync(this.projectFile)) {
      const initialProject: ProjectStatus = {
  ...
  private loadProject(): ProjectStatus {
    const data = fs.readFileSync(this.projectFile, 'utf8');
    return JSON.parse(data);
  }

  private saveProject(project: ProjectStatus): void {
    project.lastUpdated = new Date().toISOString();
    fs.writeFileSync(this.projectFile, JSON.stringify(project, null, 2));
  }

  public updateTaskProgress(taskId: string, progress: number, status?: TaskStatus['status']): void {
    ...
    for (const phase of project.phases) {
      ...
      if (task) {
        ...
        if (phase.progress === 100) {
          ...
          if (!phase.startDate) phase.startDate = new Date().toISOString();
        }
  ...
  public updateKPI(phase: number, kpiName: string, currentValue: string | number): void {
    const project = this.loadProject();
  ...
  private updateTrackerDocument(project: ProjectStatus): void {
  ...
  public showPhase(phaseNumber: number): void {
    const project = this.loadProject();
...
export { OneAgentProgressManager, ProjectStatus, PhaseProgress, TaskStatus };
