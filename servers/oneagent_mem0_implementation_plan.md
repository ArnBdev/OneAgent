...
import { UnifiedMemoryInterface, ConversationMemory, LearningMemory, PatternMemory } from './UnifiedMemoryInterface';
...
export class OneAgentMem0Bridge implements UnifiedMemoryInterface {
  ...
  private async initializeClients(): Promise<void> {
    ...
    this.memgraphDriver = new Driver(
      ...
      auth.basic(
        this.config.mem0.graph_store.username,
        this.config.mem0.graph_store.password
      )
    );
  }
  ...
  async getConversationsInWindow(timeWindow: TimeWindow): Promise<ConversationData[]> {
    ...
    try {
      ...
      return result.records.map(record => 
        this.convertToConversationData(record.toObject())
      );
    } finally {
      await session.close();
    }
