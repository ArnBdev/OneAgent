...
class EnhancedPromptEngine {
     ...
     */
    async buildEnhancedPrompt(message, memories, context, taskComplexity = 'medium') {
     ...
     */
    async validateResponseQuality(response) {
        const violations = [];
        const suggestions = [];
        let score = 100;
     ...
     */
    generateVerificationQuestions(_response, userMessage) {
        return [
            {
...
${persona.principles.map(p => `• ${p}`).join('\n')}`;
    ...
    async buildEnhancedContext(memories, context) {
        let contextSection = `
Context:
...
- Previous interactions: ${memories.length} relevant memories`;
    ...
    applyOptimalFramework(message, complexity) {
...
This response will follow the ${framework.name} structure for optimal clarity and effectiveness.`;
    ...
    buildBMADElicitation(message, _context) {
        const applicablePoints = EnhancedPromptEngine.BMAD_ELICITATION_POINTS
            .filter(point => point.applicableContexts.includes('all') ||
...
Apply this analysis framework to ensure comprehensive, high-quality responses.`;
    ...
    buildChainOfVerification(_message) {
...
Response:`;
    ...
    checkPrincipleViolation(response, principle) {
...
EnhancedPromptEngine.CONSTITUTIONAL_PRINCIPLES = [
    {
        id: 'accuracy',
...
EnhancedPromptEngine.BMAD_ELICITATION_POINTS = [
...
EnhancedPromptEngine.PROMPT_FRAMEWORKS = new Map([
    ['RTF', {
            id: 'RTF',
