...
describe('config-provider basic (canonical)', () => {
  it('applies overrides, scoped overrides and enforces freeze', async () => {
    // Base snapshot
    const snap1 = UnifiedConfigProvider.getSnapshot();
    if (!snap1.hash) throw new Error('Initial hash missing');

    // Apply runtime override
    UnifiedConfigProvider.applyRuntimeOverride(
      { memoryPort: snap1.config.memoryPort + 1 },
      { reason: 'test-runtime-override', actor: 'config-test' },
    );
    const snap2 = UnifiedConfigProvider.getSnapshot();
    if (snap2.config.memoryPort !== snap1.config.memoryPort + 1)
      throw new Error('Runtime override not applied');
    ...
    UnifiedConfigProvider.applyScopedOverride(
      'test-scope',
      { mcpPort: snap2.config.mcpPort + 5 },
      { reason: 'scoped-adjust', actor: 'config-test' },
    );
    const snap3 = UnifiedConfigProvider.getSnapshot();
    if (snap3.config.mcpPort !== snap2.config.mcpPort + 5)
      throw new Error('Scoped override not applied');
    ...
    let blocked = false;
    try {
      UnifiedConfigProvider.applyRuntimeOverride({ host: 'blocked-change' });
    } catch {
      blocked = true;
    }
    ...
    assertOperationCoverage(events, 'UnifiedConfigProvider', requiredOps, {
      allowSkipOnDisabled: true,
    });

    expect(UnifiedConfigProvider.isFrozen()).toBe(true);
  });
});
