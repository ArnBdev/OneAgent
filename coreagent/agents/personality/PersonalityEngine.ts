...
export class PersonalityEngine {
  ...
  // Canonical: Static readonly trait object is a compile-time constant, not persistent state. Not a business cache.
  private static readonly CORE_PERSONALITY_TRAITS: Record<string, PersonalityTraits[]> = {
    DevAgent: [
      {
  ...
  // Canonical: Static readonly framework object is a compile-time constant, not persistent state. Not a business cache.
  private static readonly PERSPECTIVE_FRAMEWORKS: Record<string, PerspectiveFramework> = {
    DevAgent: {
  ...
  constructor() {
    ...
    // memoryConfig removed (unused)
    this.memorySystem = getOneAgentMemory();
    // Canonical: use DI or explicit instantiation, not getInstance()
  ...
  private initializeDefaultProfiles(): void {
    ...
    for (const agentId in PersonalityEngine.CORE_PERSONALITY_TRAITS) {
      if (
        Object.prototype.hasOwnProperty.call(PersonalityEngine.CORE_PERSONALITY_TRAITS, agentId)
      ) {
        const perspectiveFramework = PersonalityEngine.PERSPECTIVE_FRAMEWORKS[agentId];
        if (perspectiveFramework) {
          // Persistent state now stored in unified cache, not local Map
          // See initializeDefaultProfiles for cache logic
        }
      }
    }
  }
  ...
  async generatePersonalityResponse(
    ...
    const profiles = (await this.cache.get('personalityProfiles')) as Record<
      string,
      PersonalityProfile
    >;
  ...
  private async storePersonalityEvolution(
    ...
    try {
      ...
      const profiles = (await this.cache.get('personalityProfiles')) as Record<
        ...
        PersonalityProfile
      >;
      const profileObj = profiles?.[agentId] || {
        agentId,
        core_traits: [],
      ...
      await this.memorySystem.addMemory({ content: response, metadata: await metadata });
    } catch (error) {
  ...
  async updatePersonalityProfile(
    ...
    let profiles = (await this.cache.get('personalityProfiles')) as Record<
      string,
      PersonalityProfile
    >;
    ...
    if (existing) {
      ...
      await this.cache.set('personalityProfiles', profiles);
