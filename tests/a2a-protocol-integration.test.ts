  ...
  try {
    ...
    console.log(
      `   Capabilities: ${Object.keys(agentCard.capabilities)
        .filter((k) => agentCard.capabilities[k as keyof typeof agentCard.capabilities])
        .join(', ')}\n`,
    );
    ...
    try {
      // This will fail since we don't have a real agent URL, but tests the method
      await protocol.sendMessageToAgent(
        `${UnifiedBackboneService.getResolvedConfig().mcpUrl}/agent`,
        testMessage,
      );
    } catch {
