...
async function runTscNoEmit() {
  return new Promise((resolve, reject) => {
    ...
    tsc.on('close', (code) => {
      ...
      else reject(Object.assign(new Error('tsc failed'), { stdout, stderr, code }));
    });
    tsc.on('error', (err) => {
      clearTimeout(timeout);
      clearInterval(progressPing);
...
async function verifyBuild() {
  ...
  try {
    ...
    async function runTsTest(file, extraEnv = {}) {
      ...
      return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
          ...
          else reject(new Error(`Test failed (${code}): ${file}\n${stderr}`));
        });
        child.on('error', (err) => {
          clearTimeout(timeout);
          reject(err);
    ...
    await runTsTest('tests/canonical/communication-conformance.test.ts', {
    ...
    await runTsTest('tests/canonical/config-provider-basic.test.ts', {
      ONEAGENT_DISABLE_AUTO_MONITORING: '',
    });
    console.log('✅ Unified config provider test passed');

    console.log('🧪 Running unified config provider advanced test...');
    await runTsTest('tests/canonical/config-provider-advanced.test.ts', {
      ONEAGENT_DISABLE_AUTO_MONITORING: '',
    });
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          'ts-node registration failed for in-process verification: ' +
            (e instanceof Error ? e.message : String(e)),
        );
      }
      const {
        unifiedAgentCommunicationService,
      } = require('../coreagent/utils/UnifiedAgentCommunicationService');
      const {
        unifiedMonitoringService,
      } = require('../coreagent/monitoring/UnifiedMonitoringService');
      ...
      const events = unifiedMonitoringService
        .getRecentEvents(200)
        .filter((e) => e.component === 'UnifiedAgentCommunicationService');
      ...
      if (missing.length)
        throw new Error('Missing required monitoring operations: ' + missing.join(', '));
      ...
      throw new Error(
        'Monitoring operation coverage verification failed: ' +
    ...
    const ops = unifiedMonitoringService
      .getRecentEvents(300)
      .filter((e) => e.component === 'UnifiedAgentCommunicationService');
