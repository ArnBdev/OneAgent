...
async function run() {
  ...
  assertOperationCoverage(recent, 'UnifiedAgentCommunicationService', requiredOps, {
  ...
  function walk(dir: string) {
    for (const entry of readdirSync(dir)) {
      ...
      if (st.isDirectory()) walk(full);
      else if (/\.tsx?$/.test(entry) && !full.endsWith('DeprecatedCommunication.ts')) {
        ...
        for (const token of forbidden) {
          ...
          if (pattern.test(txt)) {
            if (
              !full.endsWith('DeprecatedCommunication.ts') &&
              !full.endsWith('communication-conformance.test.ts')
            ) {
  ...
  walk(root);
  if (hits.length)
    throw new Error('Forbidden legacy communication pattern(s):\n' + hits.join('\n'));

  console.log('[communication-conformance.test] PASS', {
    sessionId,
    agentId,
    ops: requiredOps.length,
    monitoringAssertions:
      process.env.ONEAGENT_DISABLE_AUTO_MONITORING === '1' ? 'skipped' : 'enforced',
  });
