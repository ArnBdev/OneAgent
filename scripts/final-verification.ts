...
interface VerificationResult { name: string; status: VerificationStatus; details: string; critical: boolean; }
...
class OneAgentVerifier {
  ...
  private async verifyTypeScript() {
    try {
      ...
      this.addResult('TypeScript Compilation', 'FAIL', `Compilation failed: ${(error as Error).message}`, true);
    }
  }
  ...
  private async verifyCanonicalTypes() {
    ...
    const requiredTypes = ['MemoryRecord','MemoryMetadata','AgentResponse','UnifiedTimeContext','AgentHealthStatus'];
  ...
  private async verifyAgentCompliance() {
    ...
    for (const file of existing) {
      ...
      const requiredTokens = ['extends BaseAgent','implements ISpecializedAgent','AgentResponse','MemoryRecord'];
  ...
  private async verifyMCPServer() {
    ...
    if (!existsSync(p)) { this.addResult('MCP Server','FAIL','unified-mcp-server.ts not found',true); return; }
    const c = readFileSync(p,'utf-8');
    const ready = c.includes('json-rpc') && c.includes('http') && (c.includes('8083') || c.includes('8080'));
    this.addResult('MCP Server', ready ? 'PASS':'WARN', ready ? 'MCP server ready':'Configuration may need review', false);
  }

  private async verifyMemorySystem() {
    console.log('🧠 Verifying memory system unification...');
  const files = ['coreagent/memory/OneAgentMemory.ts','coreagent/types/oneagent-backbone-types.ts'];
  const ok = files.every(f => existsSync(join(this.projectRoot,f)));
  this.addResult('Memory System', ok ? 'PASS':'WARN', ok ? 'OneAgentMemory + backbone types present':'Missing OneAgentMemory or backbone types', false);
  }
  ...
  private async verifyConstitutionalAI() {
    ...
    if (!existsSync(p)) { this.addResult('Constitutional AI','WARN','ConstitutionalAI.ts not found',false); return; }
    const c = readFileSync(p,'utf-8');
    const ok = c.includes('validateResponse') && c.includes('ConstitutionalPrinciple') && (c.includes('85') || c.includes('threshold'));
    this.addResult('Constitutional AI', ok ? 'PASS':'WARN', ok ? 'Constitutional AI ready':'Configuration may need review', false);
  }
  ...
  const { stdout } = await execAsync('npm run -s smoke:runtime', { timeout: 180000 });
      const ok = stdout.includes('Runtime smoke passed') || stdout.includes('✓ Runtime smoke passed');
      this.addResult('Runtime Smoke', ok ? 'PASS' : 'WARN', ok ? 'End-to-end startup valid' : 'Smoke did not confirm pass; inspect logs', false);
    } catch (e) {
      this.addResult('Runtime Smoke', 'WARN', `Smoke failed to run: ${(e as Error).message}`, false);
    }
  }
