...
class AgentCommunicationProtocol {
    constructor(coreAgentId, validateWithConstitutionalAI = true) {
        this.coreAgentId = coreAgentId;
        this.validateWithConstitutionalAI = validateWithConstitutionalAI;
        this.agentRegistry = new Map();
        this.messageQueue = new Map();
        this.activeConversations = new Map();
        this.qualityThreshold = 85; // OneAgent standard
     ...
     */
    static getInstance(coreAgentId, validateWithConstitutionalAI) {
        if (!AgentCommunicationProtocol.instance) {
            AgentCommunicationProtocol.instance = new AgentCommunicationProtocol(coreAgentId || 'OneAgent-Core-v4.0.0', validateWithConstitutionalAI ?? true);
        }
        return AgentCommunicationProtocol.instance;
    }
    /**
    ...
    async queryCapabilities(query) {
        ...
        for (const [agentId, registration] of this.agentRegistry) {
            ...
            const hasMatchingCapability = registration.capabilities.some(cap => {
                ...
                return queryLower.split(' ').some(word => word.length > 3 && capLower.includes(word));
            });
            // Quality filter
     ...
     */ async coordinateAgents(task, requiredCapabilities, _context) {
        // Find suitable agents for each capability
        const agentPool = {};
        for (const capability of requiredCapabilities) {
            const agents = await this.queryCapabilities(capability);
            agentPool[capability] = agents.filter(agent => agent.status === 'online' && agent.loadLevel < 0.8);
        }
    ...
    async validateRegistrationSecurity(registration) {
        // Constitutional AI: Safety validation
        if (registration.agentId.includes('malicious') || registration.agentId.includes('hack')) {
            return false;
        }
        // Capability validation
        const hasValidCapabilities = registration.capabilities.every(cap => cap.constitutionalCompliant && cap.qualityThreshold >= 70);
    ...
    async processMessage(message, targetAgent) {
        const startTime = Date.now();
    ...
    trackConversation(message, _response) {
    ...
    async applyBMADToCoordination(_task, agentPool) {
    ...
    estimateCoordinationDuration(agentPool) {
    ...
    calculateCoordinationQuality(plan, agentPool) {
