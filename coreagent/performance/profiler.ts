...
export class PerformanceProfiler {
  ...
  constructor(monitor?: PerformanceMonitor) {
    ...
    if (!PerformanceProfiler.warned) {
      ...
      unifiedMonitoringService.trackOperation('performance_profiler', 'deprecation_notice', 'success', {
        message: 'PerformanceProfiler deprecated. Use UnifiedMonitoringService + PerformanceMonitor.'
      });
    }
  }

  startOperation(operationId: string, operationType: string, metadata?: Record<string, unknown>): void {
  ...
  endOperation(operationId: string, success: boolean = true, error?: string, metadata?: Record<string, unknown>): void {
    unifiedMonitoringService.trackOperation('performance_profiler', 'operation_complete', success ? 'success' : 'error', {
      operationId,
      phase: 'end',
      error,
      ...(metadata || {}),
  ...
  clearMetrics(): void {}
  getMetricsCount(): number { return 0; }
}

export const globalProfiler = new PerformanceProfiler();

export function createDeprecatedProfiler(): PerformanceProfiler {
  unifiedMonitoringService.trackOperation('performance_profiler', 'deprecated_factory_call', 'success', {
