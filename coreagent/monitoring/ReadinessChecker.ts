...
export async function generateReadinessReport(): Promise<ReadinessReport> {
  ...
  const mcpUrl = environmentConfig.endpoints.mcp.url.replace(/\/$/, '') + environmentConfig.endpoints.mcp.path;
  ...
  const overall: ReadinessReport['overall'] = allHealthy ? 'healthy' : anyHealthy ? 'degraded' : 'unhealthy';
  return { overall, systems, timestamp: createUnifiedTimestamp().iso };
}

export async function waitForReadiness(timeoutMs = 15000, intervalMs = 1500): Promise<ReadinessReport> {
  const start = Date.now();
