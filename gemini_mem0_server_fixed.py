...
class MemoryRequestHandler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if path == '/health':
                self._send_json_response({
                  ...
                  elif path.startswith('/memories') or path.startswith('/v1/memories'):
                # Get memories - support both /memories and /v1/memories/ paths
    ...
    def do_POST(self):
        ...
        try:
            parsed_url = urlparse(self.path)            path = parsed_url.path
            body = self._parse_request_body()
            
            if path == '/memories' or path == '/v1/memories' or path == '/v1/memories/':
                # Add memory - support both /memories and /v1/memories/ paths
