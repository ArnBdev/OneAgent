...
export class ConstitutionalAIValidator {
  ...
  constructor() {
    ...
    console.log('✅ Constitutional AI Validator initialized with OneAgent integration');
  }
  ...
  async validateMessage(context: ValidationContext): Promise<ConstitutionalValidation> {
    ...
    console.log(`🔍 Constitutional AI validation started: ${validationId}`);

    try {
      // Check cache for recent similar validations
      const cacheKey = this.generateCacheKey(context);
      if (this.validationCache.has(cacheKey)) {
        console.log(`⚡ Using cached validation for similar content`);
      ...
      if (validation.overallScore >= 0.8) {
        // Only store high-quality validations
        await this.storeValidationInMemory(validation, context);
      }

      console.log(
        `✅ Constitutional AI validation complete: ${validation.qualityGrade} (${Math.round(validation.overallScore * 100)}%)`,
      );
      return validation;
    } catch (error) {
      console.error(`❌ Constitutional AI validation failed: ${error}`);
  ...
  private async assessTransparency(context: ValidationContext): Promise<number> {
    ...
    const acknowledgesLimitations = limitationPhrases.some((phrase) =>
      content.toLowerCase().includes(phrase),
    );
  ...
  private async assessSafety(context: ValidationContext): Promise<number> {
    ...
    const hasWarnings =
      content.includes('warning') || content.includes('caution') || content.includes('be careful');
  ...
  private async storeValidationInMemory(
    ...
    console.log(`💾 Constitutional AI validation stored in memory: ${validation.validationId}`);
  }
  ...
  async validateBatch(contexts: ValidationContext[]): Promise<ConstitutionalValidation[]> {
    console.log(`🔍 Batch Constitutional AI validation started: ${contexts.length} messages`);

    const validations = await Promise.all(contexts.map((context) => this.validateMessage(context)));

    const averageScore =
      validations.reduce((sum, v) => sum + v.overallScore, 0) / validations.length;
    console.log(`✅ Batch validation complete: Average score ${Math.round(averageScore * 100)}%`);
...
3. **Integrate with NLACS messaging**:
   ...
   const validation = await validateConstitutionalAI(message.content, message.fromAgent, {
     nlacs: true,
   ...
   if (validation.qualityGrade === 'D') {
     throw new Error(
       `Constitutional AI validation failed: ${validation.violations.map((v) => v.description).join(', ')}`,
     );
   }
   ```
