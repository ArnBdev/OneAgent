...
const UnifiedMCPTool_1 = require("./UnifiedMCPTool");
const index_1 = require("../config/index");
class MemoryCreateTool extends UnifiedMCPTool_1.UnifiedMCPTool {
    constructor() {
        const schema = {
            type: 'object',
            properties: {
        ...
        super('oneagent_memory_create', 'Create new memory with real-time learning capability', schema, 'enhanced' // Constitutional AI level
        );
    } /**
    ...
    async executeCore(args) {
        try {
            ...
            const { realUnifiedMemoryClient } = await Promise.resolve().then(() => __importStar(require('../memory/RealUnifiedMemoryClient')));
            ...
            const enhancedMetadata = {
                ...
                ...(Object.fromEntries(Object.entries(args.metadata || {}).filter(([key, value]) => key !== 'priority' && // Handled specially above
            ...
            const result = await realUnifiedMemoryClient.createMemory(args.content, args.userId, memoryType, enhancedMetadata);
            if (!result.success) {
    ...
    getFallbackData() {
        return {
            ...
            suggestions: [
                `Verify unified memory server is running on port ${index_1.oneAgentConfig.memoryPort}`,
