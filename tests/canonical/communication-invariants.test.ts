...
process.env.ONEAGENT_FAST_TEST_MODE = '0';
...
describe('UnifiedAgentCommunicationService invariants', () => {
  afterAll(async () => {
    const { AgentFactory } = await import('../../coreagent/agents/base/AgentFactory');
    await AgentFactory.shutdownAllAgents();
  });

  it('tracks send/receive counts, broadcast and handler lifecycle', async () => {
    const { AgentFactory } = await import('../../coreagent/agents/base/AgentFactory');
    ...
    try {
      ...
      expect(counters.registered).toBeGreaterThanOrEqual(2);
      expect(counters.sent).toBe(sentBeforeOff);
      expect(counters.received).toBe(recvBeforeOff);
    } catch (err) {
