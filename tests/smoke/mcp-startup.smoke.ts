...
async function run() {
  ...
  assert.ok(info.server && info.server.protocol && info.server.version, 'info should contain server metadata');
  ...
  assert.ok(toolsList.result && Array.isArray(toolsList.result.tools), 'tools/list should return tools');
