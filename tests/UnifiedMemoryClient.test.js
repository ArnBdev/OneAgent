...
exports.runUnifiedMemoryClientTests = runUnifiedMemoryClientTests;
const UnifiedMemoryClient_1 = require("./UnifiedMemoryClient");
/**
 * Test configuration
 */
const TEST_CONFIG = {
 ...
 * Main test runner
 */
async function runUnifiedMemoryClientTests() {
    console.log('🚀 Starting UnifiedMemoryClient Test Suite...\n');
    const client = new UnifiedMemoryClient_1.UnifiedMemoryClient(TEST_CONFIG);
    let testResults = {
        passed: 0,
        failed: 0,
        total: 0
 ...
 */
async function testClientInitialization(client, results) {
 ...
 */
async function testConnectionManagement(client, results) {
 ...
 */
async function testMemoryOperations(client, results) {
    console.log('📋 Test 3: Memory Operations');
    try {
        // Test create memory
        const createResult = await client.createMemory(TEST_MEMORY_CONTENT, TEST_USER_ID, 'long_term', {
            category: 'test',
        ...
        const searchResult = await client.getMemoryContext(TEST_QUERY, TEST_USER_ID, 5);
 ...
 */
async function testConstitutionalValidation(client, results) {
    console.log('📋 Test 4: Constitutional AI Validation');
    try {
        const validationResult = await client.validateConstitutional(TEST_MEMORY_CONTENT, { source: 'test' });
 ...
 */
async function testQualityScoring(client, results) {
    console.log('📋 Test 5: Quality Scoring');
    try {
        const qualityResult = await client.generateQualityScore(TEST_MEMORY_CONTENT, ['accuracy', 'clarity', 'completeness']);
 ...
 */
async function testPerformanceMetrics(client, results) {
 ...
 * Test error handling
 */
async function testErrorHandling(client, results) {
 ...
 */
async function testMCPCompliance(client, results) {
...
//# sourceMappingURL=UnifiedMemoryClient.test.js.map