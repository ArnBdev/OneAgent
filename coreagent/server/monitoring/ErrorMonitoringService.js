...
class ErrorMonitoringService {
    constructor(constitutionalAI, auditLogger, triageAgent) {
     ...
     */
    async assessConstitutionalCompliance(error, context) {
        try {
            const errorResponse = `Error occurred: ${error.message}. Context: ${JSON.stringify(context)}`;
            const validation = await this.constitutionalAI.validateResponse(errorResponse, 'System error assessment', context);
     ...
     */
    async triggerTriageIntervention(report) {
        if (!this.triageAgent) {
            return false;
        }
        try {
     ...
     * Log error with Constitutional AI compliance
     */
    async logErrorWithCompliance(report) {
     ...
     */
    getErrorPatternCount(errorMessage) {
     ...
     */
    clearOldReports(olderThan = 24 * 60 * 60 * 1000) {
