...
export function computeOperationCoverage(
  ...
  const present = Array.from(new Set(componentEvents.map(e => e.operation).filter(Boolean))) as string[];
...
export function assertOperationCoverage(
  ...
  if (process.env.ONEAGENT_DISABLE_AUTO_MONITORING === '1' && opts.allowSkipOnDisabled) {
    console.log(`[monitoringTestUtils] Monitoring disabled; skipping operation coverage assertion for ${component}`);
    return;
  }
  const result = computeOperationCoverage(events, component, requiredOps);
  if (result.missing.length) {
    throw new Error(`Missing monitoring operation events for component ${component}: ${result.missing.join(', ')} | Present: ${result.present.join(', ')}`);
  }
  console.log(`[monitoringTestUtils] Operation coverage 100% for ${component}`, { present: result.present });
}

/**
 * Convenience helper to filter events to only those whose operation is in the provided list.
 */
export function getAndFilterEvents(events: MonitoringEvent[], operations: string[]): MonitoringEvent[] {
 ...
 */
export function assertOperationSequence(events: MonitoringEvent[], component: string, sequence: string[]): void {
  const componentOps = events.filter(e => e.component === component && e.operation).map(e => e.operation as string);
  let idx = 0;
  ...
  if (idx !== sequence.length) {
    throw new Error(`Operation sequence not observed for ${component}. Expected subsequence: ${sequence.join(' -> ')} | Observed ops: ${componentOps.join(', ')}`);
  }
