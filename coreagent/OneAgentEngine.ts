...
export class OneAgentEngine extends EventEmitter {
  ...
  private async initializeMultiAgentSystem(): Promise<void> {
    ...
    try {
      // Initialize NLACS orchestrator first
      this.multiAgentOrchestrator = UnifiedNLACSOrchestrator.getInstance();
      await this.multiAgentOrchestrator.initialize();
      
      // Set NLACS as the communication protocol for agent bootstrap service
      // This connects the agents to use NLACS for inter-agent communication
      await agentBootstrap.setCommunicationProtocol(this.multiAgentOrchestrator as any);
      
      // Bootstrap agents - they will now register with NLACS
      await agentBootstrap.bootstrapAllAgents();
      
      console.log('✅ Multi-agent system initialized with NLACS communication protocol');
  ...
  private async handleOneAgentTool(method: string, params: any): Promise<any> {
    switch (method) {
      ...
      case 'oneagent_bmad_analyze':
        // Use the actual BMAD engine with correct method
      ...
      case 'oneagent_quality_score':
        // Generate quality score using Constitutional AI
        const validation = await this.constitutionalAI.validateResponse(
          params.content, 
          'Quality assessment'
        );
      ...
      case 'get_agent_network_health':
        ...
        return this.multiAgentOrchestrator.getNetworkHealth();
