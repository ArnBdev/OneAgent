...
const memStats = (userId = 'smoke-user') => `http://127.0.0.1:${memPort}/v1/memories/stats?userId=${encodeURIComponent(userId)}`;
...
function httpGetWithHeaders(url: string, headers: Record<string, string>, timeoutMs = 5000): Promise<{ status: number; body: string; headers: http.IncomingHttpHeaders }> {
  return new Promise((resolve, reject) => {
    const u = new URL(url);
    const req = http.request({ hostname: u.hostname, port: u.port, path: `${u.pathname}${u.search}`, method: 'GET', timeout: timeoutMs, headers }, res => {
      res.setEncoding('utf8');
      let raw = '';
      res.on('data', (d: string) => { raw += d; });
...
interface JsonRpcResponse { jsonrpc?: string; id?: number | string | null; result?: unknown; error?: unknown }
  ...
  if (!memHealthJson.success) throw new Error('Memory /health not healthy');
    ...
    if (memKey) {
      try {
        ...
        const statsRes = await httpGetWithHeaders(memStats('smoke-user'), authHeaders as unknown as Record<string, string>);
        if (statsRes.status === 200) {
          ...
          console.warn('Memory stats unauthorized or protocol mismatch; skipping optional memory op.');
        } else {
    ...
    if (!infoJson.server || !infoJson.server.version) throw new Error('MCP /info missing server metadata');

    // MCP initialize
  const initRes = await httpPostJson(mcpUrl, {
      jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'runtime-smoke', version: '0.0.0' } }
    });
  const initOk = Boolean(initRes && typeof initRes === 'object' && initRes.result && typeof initRes.result === 'object' && (initRes.result as { serverInfo?: unknown }).serverInfo);
  if (!initOk) throw new Error('MCP initialize failed');

    // tools/list
  const tools = await httpPostJson(mcpUrl, { jsonrpc: '2.0', id: 2, method: 'tools/list' });
  const toolsOk = Boolean(tools && typeof tools === 'object' && tools.result && Array.isArray((tools.result as { tools?: unknown[] }).tools));
