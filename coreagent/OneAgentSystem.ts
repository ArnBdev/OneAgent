...
export class OneAgentSystem extends EventEmitter {
  ...
  private async executeHandoff(targetAgentId: string): Promise<string> {
    ...
    this.addToConversationHistory(
      'system',
      `Handoff from ${previousAgent.id} to ${targetAgent.id}`,
    );
  ...
  private async initiateTeamMeeting(request: TeamMeetingRequest): Promise<string> {
    try {
      ...
      this.addToConversationHistory(
        'team-meeting',
        `Meeting conducted: ${meetingResult.synthesis?.keyInsights.join(', ')}`,
      );
...
class CoreAgent implements SpecialistAgent {
  ...
  private async generateDirectResponse(
    ...
    try {
      const response = await this.existingCoreAgent.processMessage(
        {
          user: {
            id: context.userId,
            name: 'User',
...
class TeamMeetingEngine {
  ...
  private inferParticipantsFromTopic(topic: string): string[] {
    ...
    if (
      ...
      topicLower.includes('develop') ||
      topicLower.includes('architecture')
    ) {
      participants.push('DevAgent');
    }

    if (
      topicLower.includes('document') ||
  ...
  private async facilitateDiscussion(meeting: TeamMeeting): Promise<void> {
    ...
    for (const question of questions.slice(0, 2)) {
      // Limit to 2 rounds for efficiency
      for (const participantId of meeting.participants) {
        try {
          const agent =
            this.oneAgentSystem.specialists.get(participantId) ||
  ...
  private async synthesizeResults(meeting: TeamMeeting): Promise<void> {
    ...
    const avgConfidence =
      meeting.perspectives.reduce((sum, p) => sum + p.confidence, 0) / meeting.perspectives.length;
  ...
  private detectConflicts(perspectives: AgentPerspective[]): string[] {
    ...
    if (
      recommendations.some((r) => r.includes('avoid')) &&
      recommendations.some((r) => r.includes('implement'))
    ) {
  ...
  private resolveConflicts(conflicts: string[]): string[] {
    return conflicts.map(
      (conflict) => `Resolution: Consider phased approach or hybrid solution for: ${conflict}`,
    );
  }
