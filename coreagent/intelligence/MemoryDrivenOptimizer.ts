...
export class MemoryDrivenOptimizer {
  ...
  async monitorOptimization(): Promise<{
    ...
    const activeOptimizations = Array.from(this.optimizationProfiles.values()).filter(
      (profile) => profile.currentState === 'optimizing' || profile.currentState === 'monitoring',
    );
      ...
      if (metric in targets) {
        ...
        if (value < target) {
          gaps.push({
            ...
            potentialStrategies: this.suggestStrategiesForMetric(
  ...
  private calculateAnalysisConfidence(metrics: PerformanceMetrics, gaps: PerformanceGap[]): number {
    ...
    const gapClarity =
      gaps.length > 0 ? gaps.reduce((sum, gap) => sum + gap.gap, 0) / gaps.length : 0.5;
  ...
  private generateMonitoringRecommendations(metrics: StrategyResult[]): string[] {
    ...
    const avgEffectiveness =
      metrics.reduce((sum, m) => sum + m.effectivenessRatio, 0) / metrics.length;
  ...
  private async identifyWorkflowOptimizations(request: {
    domain: string;
    currentWorkflow: string;
    performanceData: { averageTime: number; successRate: number; bottlenecks: string[] };
  }): Promise<string[]> {
  ...
  private calculateExpectedImprovement(performanceData: {
    bottlenecks: string[];
    successRate: number;
  }): number {
  ...
  private async searchMemoryForInsights(request: {
    domain: string;
    timeframe: string;
    focus: string;
  }): Promise<
  ...
  private analyzeSuccessPatterns(
    memoryData: Array<{ metadata?: { confidence?: number; quality?: number } }>,
  ): string[] {
    const patterns: string[] = [];

    const highQualityItems = memoryData.filter(
      (item) => (item.metadata?.confidence ?? 0) > 0.8 || (item.metadata?.quality ?? 0) > 0.8,
    );
  ...
  private analyzeTemporalPatterns(
    memoryData: Array<{ timestamp?: number | string | Date }>,
  ): string[] {
    const patterns: string[] = [];
